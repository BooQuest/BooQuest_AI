...
스텝은 순차적으로 진행되어야 하며, 이전 스텝을 완료해야 다음 스텝으로 진행할 수 있어야 합니다.
    ...
    def regenerate_sidejob_prompt_by_feedback(requestDto: RegenerateSideJobRequest) -> str:
        ...
        with open(platform_path, "r", encoding="utf-8") as f:
            PLATFORM_SIDE_JOBS = json.load(f)

        with open(sns_path, "r", encoding="utf-8") as f:
            SNS_SIDE_JOBS = json.load(f)


        allowed_platforms = [platform for v in PLATFORM_SIDE_JOBS.values() for platform in v]
        allowed_formats = [category for v in SNS_SIDE_JOBS["category"].values() for category in v]
        ...
        - [형식]: 아래 목록 중 하나에서 선택 (실제 형식 카테고리 기반)
        {json.dumps(allowed_formats, ensure_ascii=False, indent=2)}
        - [플랫폼]: 아래 목록 중 하나에서 선택 (실제 플랫폼 기반)
        {json.dumps(allowed_platforms, ensure_ascii=False, indent=2)}
