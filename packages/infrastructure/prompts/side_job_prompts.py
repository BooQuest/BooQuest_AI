...
class SideJobPrompts:
    ...
    def create_prompt_template(self, job: str, hobbies: list, expression_style: str, strength_type: str) -> ChatPromptTemplate:
        """사이드잡 생성 프롬프트 템플릿을 생성합니다."""
        ...
        system_prompt = """당신은 사용자의 직업, 취미, 표현 스타일, 강점을 분석하여 맞춤형 사이드잡을 추천하는 AI 어시스턴트입니다.

사용자의 특성을 고려하여 실현 가능하고 구체적인 사이드잡을 제안해야 합니다.
...
위 정보를 바탕으로 사용자에게 적합한 사이드잡 3개를 제안해주세요.

**중요: 반드시 각 사이드잡에 title과 description 필드를 모두 포함해야 합니다.**
...
**주의사항:**
1. title과 description 필드는 반드시 모든 사이드잡에 포함되어야 합니다
2. description 필드가 비어있거나 누락되면 안 됩니다
3. JSON 형식이 정확해야 합니다
